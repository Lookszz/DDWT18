...
function get_navigation($navigation_array, $active_id){
    ...
    foreach ($navigation_array as $id => $template) {
        if ($id == $active_id){
            $navigation_exp .= '<li class="nav-item active">';
            $navigation_exp .= '<a class="nav-link" href="'.$navigation_array[$active_id]['url'].'">'.$navigation_array[$active_id]['name'].'</a>';
        }else if ($id != $active_id){
            $navigation_exp .= '<li class="nav-item">';
            $navigation_exp .= '<a class="nav-link" href="'.$navigation_array[$id]['url'].'">'.$navigation_array[$id]['name'].'</a>';
        }
...
function get_serie_table($pdo, $series){
    ...
    foreach($series as $key => $value){
        ...
        <tr>
            ...
            <th scope="row">'.get_user_name($pdo, $value['user'])['firstname']. ' '.get_user_name($pdo, $value['user'])['lastname'].'</th>
...
/**
 * Get current user_name with specific user id
 * @param object $pdo database object
 * @return array with name of the user
 */
...
function register_user($pdo, $form_data){
    ...
    $feedback = [
        'type' => 'success',
        'message' => sprintf('%s, your account was successfully created!', get_user_name($pdo, get_user_id()))
    ];
...
function login_user($pdo, $form_data){
    ...
    if ( !password_verify($form_data['password'], $user_info['password']) ){
        ...
        $user_id = get_user_id();
        $name = get_user_name($pdo, $user_id);
        $feedback = [
            'type' => 'success',
            'message' => sprintf('%s, you were logged in successfully!',
                get_user_name($pdo, implode(" ", $name)))
        ];
        redirect(sprintf('/DDWT18/week2/myaccount/?error_msg=%s',
            json_encode($feedback)));
    }
